...
private:
  ...
  void pullRx_(){ if(!acm_) return; uint8_t buf[64]; for(;;){ uint16_t len=sizeof(buf); uint8_t r=acm_->RcvData(&len, buf); lastRcvErr_=r; if(r||!len) break; bytesIn_+=len; for(uint16_t i=0;i<len;i++) rx_.push_back(buf[i]); ring_.push(buf,len); if(rx_.size()>16384){ size_t excess = rx_.size() - 8192; rx_.erase(rx_.begin(), rx_.begin() + excess); } } }
